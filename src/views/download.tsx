...
export default function DownloadView({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [openFileState, openFileFn] = useAsyncFn(async function openFile(wordCode: string) {
    ...
    let exists;
    try {
      exists = await FileService.doesExist(wordCode);
    } catch (e) {
      ErrorHandlingService.notifyUserOfError("Error checking for file existance", e);
      return;
    }
    ...
    <>
      <Modal isCentered isOpen={isOpen} onClose={_onClose} initialFocusRef={initialRef as any}>
        ...
        <ModalContent mx={3}>
          ...
          <ModalCloseButton ref={initialRef as any} onClick={_onClose} />
          <ModalBody>
            <form
              onSubmit={async e => {
                e.preventDefault();
