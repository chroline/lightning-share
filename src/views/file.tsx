...
export default function FileView({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const fileState = useAsync(async () => {
    if (!wordCode) return;
    try {
      return await FileService.getInfo(wordCode);
    } catch (e) {
      ErrorHandlingService.notifyUserOfError("Error retrieving file information", e);
      onClose();
    }
  }, [wordCode]);
  ...
  const [downloadFileState, downloadFileFn] = useAsyncFn(async function downloadFile(
    ...
    try {
      await FileService.download(wordCode, filename);
    } catch (e) {
      ErrorHandlingService.notifyUserOfError("Error downloading file", e);
    }
  });
  ...
  const [removeFileState, removeFileFn] = useAsyncFn(async function removeFile(wordCode: string, filename: string) {
    try {
      await FileService.remove(wordCode, filename);
      closeRemoveModal();
      onClose();
    } catch (e) {
      ErrorHandlingService.notifyUserOfError("Error removing file", e);
    }
  });
  ...
  function copyWordCode() {
    try {
      ...
      ErrorHandlingService.notifyUserOfError("Copying word code unsuccessful", e);
    }
  }
      ...
      <Drawer placement={"right"} onClose={onClose} isOpen={isOpen} initialFocusRef={initialRef as any}>
        ...
        <DrawerContent>
          ...
          <DrawerFooter borderTopWidth={"1px"}>
            {fileState.value?.owner === FirebaseService.userId$.value && (
              <Button
                colorScheme={"red"}
                variant={"outline"}
                mr={3}
